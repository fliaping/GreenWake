...
import (
	...
	"greenwake-guard/pkg/autostart"
	"greenwake-guard/pkg/dialog"
...
func NewTrayService(wakeLockService *wakelock.Service) *TrayService {
	autoStartMgr, err := autostart.NewManager("greenwake-guard", nil)
	if err != nil {
...
func (s *TrayService) Start() {
	...
	if _, err := os.Stat(langDir); os.IsNotExist(err) {
		...
		// 尝试在 Contents/Resources/assets/lang 目录下查找
		langDir = filepath.Join(baseDir, "..", "Resources", "assets", "lang")
		if _, err := os.Stat(langDir); os.IsNotExist(err) {
			// 如果还是找不到，尝试在 Contents/MacOS/assets/lang 目录下查找
	...
	s.app = app.NewWithID("com.fliaping.greenwakeguard")
	// 设置应用跟随系统主题
