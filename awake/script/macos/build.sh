...
cd "$PROJECT_ROOT"
CGO_ENABLED=1 GOARCH=amd64 go build -ldflags "-s -w -X main.Version=$VERSION" -o "$TEMP_DIR/$APP_NAME-amd64" cmd/awake/main.go

# 编译 Apple Silicon 版本
echo "编译 Apple Silicon (arm64) 版本..."
CGO_ENABLED=1 GOARCH=arm64 go build -ldflags "-s -w -X main.Version=$VERSION" -o "$TEMP_DIR/$APP_NAME-arm64" cmd/awake/main.go

# 合并为 Universal Binary
echo "创建 Universal Binary..."
lipo -create -output "$MACOS_BIN_DIR/$APP_NAME" "$TEMP_DIR/$APP_NAME-amd64" "$TEMP_DIR/$APP_NAME-arm64"
...
<dict>
    ...
    <string>com.fliaping.awake</string>
...
# 重命名最终的 DMG 文件
mv "$BUILD_DIR/$DMG_NAME.dmg" "$BUILD_DIR/Awake.dmg"

echo "打包完成！"
echo "DMG 包位置: $BUILD_DIR/Awake.dmg" 