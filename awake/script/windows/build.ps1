...
$env:CGO_ENABLED = "1"
$env:GOOS = "windows"
go build -ldflags "-s -w -H=windowsgui -X main.Version=$VERSION" -o "$DIST_DIR\awake.exe" cmd/awake/main.go
Pop-Location
...
#define MyAppURL "https://github.com/fliaping/awake"
#define MyAppExeName "awake.exe"

[Setup]
AppId={{A8F68DCB-B610-478B-9D35-B1E0C0E3EFBB}
AppName={#MyAppName}
...
AllowNoIcons=yes
OutputDir=$BUILD_DIR
OutputBaseFilename={#MyAppName}_Setup_{#MyAppVersion}_$ARCH
Compression=lzma
...
[Files]
Source: "$DIST_DIR\awake.exe"; DestDir: "{app}"; Flags: ignoreversion
...
Write-Host "打包完成！"
Write-Host "安装包位置: $BUILD_DIR\${APP_NAME}_Setup_${VERSION}_${ARCH}.exe" 